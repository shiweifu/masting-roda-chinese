...
在前面小节，我们讨论了许多 Roda 用于渲染页面和响应的插件。在本节，我们将讨论扩展 `Roda` 路由功能的一些插件。
...
大多数应用程序开始时候规模都很小。如果我们幸运，我们的应用程序会一直停留在小规模。然而，应用往往规模越来越大。我们也许开始只有五个路由，在加了一些功能后，应用有了 100 个路由。



在 `Roda` 的核心，全部的路由在相同的块中，块不能被拆分到 Ruby 文件中，所以，意味着所有路由只能在一个文件中。然而，对于规模大一些的程序，这不是一个好的实践。Roda 意识到了这一点，为规模巨大的应用引入了插件。在本节，我们将讨论处理规模大的情况下需要用到的插件 -- `hash_routes`。
...
默认情况下，当 Roda 无法处理路由时，Roda 将返回 404。在主流浏览器中，收到 404 会返回特别的页面，少数浏览器，会返回空白页面。



我们可以通过 `not_found` 插件来定制 404 页面。我们像该插件传递匹配块，在这个块中，对 404 进行响应（例如当我们不处理路由树中的路由时）被调用。与任何匹配块一样，该块的返回值做为响应的主题。在本例中，我们希望所有未处理的响应都是用应用的标准布局，并显示 `views/404.erb` 文件。
...
通常来说，将同一路径后面的任意地址不加区分的处理，是一个坏主意。一般来说，将 `/on` 和 `/on/` 视为同一路径，也是一个坏主意。他们是不同的地址，不应该被视为相同路径。如果将他们视为相同路径，有一个问题是，这样的资源，相对路径的工作方式不同，因为 `/on` 使用前面的 `/`，`/on/` 使用 `/on/`。



因此，如果我们想将多个路由作统一处理，则需要使用单独的路由块，其中一个保留路径为空（r.get true），另一个保留路径为 /（r.root）。
...
上面的代码可以工作，但是有一点奇怪。它首先测试路径的下一节是 `/`。如果成立，将被消耗掉。如果不成立，将转到下一个匹配器 `true`，它始终被匹配，且不被消耗。在空路径和 `/` 剩余路径的情况下，匹配器都经过处理后，剩余路径将被完全消耗。



`empty_root` 插件允许我们简化 `r.get ["", true]` 调用，来使 `r.root` 同时匹配 `/` 剩余路径和空路径。
...
请记住，使用 `empty_root` 可能是一种反模式。仅当我们要使用将两条截然不同的路径（尾部斜线和尾部无斜线）视为同一路径时，才使用该插件。
...
`from`，`to`，以及 `subject` 方法用于设置邮件的 `From`，`To` 和 `Subject` 头。如上面的例子所示，我们可以调用在路由树的任何地方。上面的代码中，演示了同样的寄件人和地址，被用于所有的邮件任务。当在 `r.on "tasks", Integer` 分之中，调用寄件人和收件人方法，为指定的任务设置不同的信息。



`Roda` 的 `mailer` 插件的特殊之处，在于我们可以在其中使用大多数 Roda 的其他插件。在大多数情况下，我们不想在路由树中，指定邮件的正文，通常正文都很大。就像 Web 请求一样，我们使用单独的方法创建他们，两种情况下，我们可以使用相同的插件，即 `render` 插件。我们还可以使用  `symbol_views` 之类的插件，来使渲染代码清爽，这一切就像在路由树中处理 Web 请求一样。
...
使用路径发送邮件，允许 `Mailer` 对象使用路由树来进行处理，这种方式具备本书讨论的其他优点。而使用路径发送邮件的另一个好处是，可以很容易的引入用于发送邮件的作业系统，稍后将讨论使用作业系统发送邮件。使用这种方式具备很好的伸缩性，对 Web 请求可以进行响应，而不必等邮件发送完成。使用地址发送邮件，也可以确保切换到作业系统后，不会出现序列化问题。



除了 `from`，`to` 和 `subject` 方法外，`mailer` 插件同样支持 `cc` 和 `bcc` 方法，用于设置抄送。以及使用 `add_file` 方法，支持添加附件。`mailer` 还支持多邮件部分，通过 `html_part` 和 `text_plain` 方法，设置纯文本内容和富文本内容。对于需要将 `Web` 请求转换为邮件的情况，我们可以为 `sendmail` 调用提供其他参数，这会需要其他参数。具体情况请查阅 `mailer` 插件的相关文档。
...
`mailer` 插件用于处理发送邮件，但是如何我们想收取邮件，并根据收取的邮件执行某些处理呢？`Roda` 也有相应的插件，叫做 `mail_processor`。像 `Roda` 核心和 `mailer` 插件一样，本插件也使用 `Roda` 路由树的形式处理邮件，但是因为接收邮件没有路径，处理的路由方法写起来有一些困难，需要换一种形式。



关于 `mail_processor` 插件，我们在请求实例上，有路由方法，就像 `Roda` 核心。但是，路由方法是根据电子邮件的组成部分明明的，所以，最常见的方法是 `r.from`，`r.rcpt`，`r.subject` 以及 `r.text`。由于邮件处理不需要返回结果，因此 `mail_processor` 插件使用 `r.handle` 方法，显示的标记邮件的处理状态。所有的路由方法都有快捷方式，在同一方法调用中，结合了处理和匹配。
...
`mail_processing` 插件提供对 `hooks` 的支持，无论邮件是否处理，都可以使用。它还支持自定义回复邮件的文本提取。有关这些功能的详细介绍，请查看 `mail_processor` 插件文档。
